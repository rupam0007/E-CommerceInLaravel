            ...
            <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ route('home') }}" class="font-serif text-3xl font-bold text-indigo-400">
                            Nexora
                        </a>
                    </div>
                    ...
                    <div class="hidden sm:ml-6 sm:flex sm:items-center space-x-8">
                        ...
                        <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-300 hover:text-indigo-400 transition-colors {{ request()->routeIs('products.index') ? 'text-indigo-400' : '' }}">
                            Shop
                        </a>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="{{ route('register') }}"
                            class="ml-4 text-sm font-medium text-white bg-indigo-500 px-4 py-2 rounded-md hover:bg-indigo-600 transition-colors shadow-sm">
                            Register
                        </a>
                        @endif
                        @else
                        ...
                        <div class="relative ml-3">
                            ...
                            <div id="user-menu" class="hidden origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none border border-gray-700" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                <div class="px-4 py-2 text-sm text-gray-300">
                                    <div>{{ Auth::user()->name }}</div>
                                    <div class="font-medium truncate">{{ Auth::user()->email }}</div>
                                </div>
                                <hr class="border-gray-700">
                                <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700" role="menuitem" tabindex="-1">My Profile</a>
                                <a href="{{ route('orders.index') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700" role="menuitem" tabindex="-1">My Orders</a>

                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type-="submit" class="w-full text-left block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700" role="menuitem" tabindex="-1">
                                        Log Out
                                    </button>
                                </form>
                            </div>
